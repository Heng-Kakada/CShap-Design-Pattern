...
{
    public SugarMixing(AbstractDrink comp, string name, double weight) : base(comp)
    {
        m_name = name;
        if (m_name == null || string.IsNullOrEmpty(m_name.Trim()))
        {
            m_name = "Unknown Surgar";
        }
        m_weight = weight;
    }
    public override object GetDescription(double weight)
    {
        ...
        return string.Format("{0}Surgar({1}, {2:N2} Gs, {3:P2})", temp, m_name, m_weight, per);
    }
